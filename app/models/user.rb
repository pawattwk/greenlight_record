...
class User < ApplicationRecord
  ...
  validates :tellnumber, length: { minimum: 10, maximum: 10 },uniqueness: { case_sensitive: false, scope: :provider }
  ...
  def generate_token_login
    gen_token = loop do
      ...
      break random_token unless User.exists?(login_token: random_token)
    end
  end
