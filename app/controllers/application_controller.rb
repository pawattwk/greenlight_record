...
class ApplicationController < ActionController::Base
  ...
  def current_user
    @current_user ||= User.includes(:roles, :main_room).find_by(id: session[:user_id])
     ...
     #check login overlap
    #  if !session[:login_token].nil? 
    #   @login_token = User.where(id: session[:user_id], login_token: session[:login_token]).select(:id,:login_token).first
    #   if @login_token.nil? 
    #     #logout
    #     session.delete(:login_token)
    #     session.delete(:user_id)
    #     redirect_to root_path , flash: { alert: I18n.t("login_over.overlap") }
    #   end
    # end
