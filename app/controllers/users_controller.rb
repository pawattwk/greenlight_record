...
class UsersController < ApplicationController
  ...
  def roomuid_api
    @user_id = User.where(email: params[:email]).select(:id).first
    @room_uid = Room.where(user_id: @user_id.id).select(:uid).first
    if !@room_uid.nil?
      render json: {status: 'Success',uid: @room_uid.uid }
    end
  end
  ...
  def create
      ...
      redirect_to root_path , flash: { success: ("Register Success") }
  end
  ...
  def user_params
    params.require(:user).permit(:name, :email, :image, :password, :password_confirmation,
      :new_password, :provider, :accepted_terms, :language, :tellnumber)
